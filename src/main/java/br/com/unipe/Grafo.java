...
public class Grafo {
    ...
    public List<String> getNomesAdjacencias(String nome) {
        Vertice vertice = encontraVertice(nome).orElseThrow(
                () -> new IllegalArgumentException("Vertice " + nome + " nao encontrado."));

        return vertice.getAdjacencias().stream()
                .map(Vertice::getNome)
                .toList();
    }

    public List<String> buscaEmLargura(String origem) {
        Vertice verticeOrigem = encontraVertice(origem).orElseThrow(
                () -> new IllegalArgumentException("Vertice " + origem + " nao encontrado."));
        ...
        while (!fila.isEmpty()) {
            ...
            ordemVisita.add(atual.getNome());

            for (Vertice vizinho : getAdjacenciasOrdenadas(atual)) {
    ...
    public int buscaEmLargura(String origem, String destino) {
        ...
        Vertice verticeDestino = encontraVertice(destino).orElseThrow(
                () -> new IllegalArgumentException("Vertice " + destino + " nao encontrado."));
    ...
    public ResultadoCaminho dijkstra(String origem, String destino) {
        ...
        PriorityQueue<VerticeDistancia> fila = new PriorityQueue<>(
                Comparator.comparingInt(VerticeDistancia::getDistancia)
                        .thenComparing(item -> item.getVertice().getNome()));
        ...
        distancias.put(verticeOrigem, 0);
        fila.add(new VerticeDistancia(verticeOrigem, 0));

        while (!fila.isEmpty()) {
            VerticeDistancia itemAtual = fila.poll();
            Vertice atual = itemAtual.getVertice();

            if (itemAtual.getDistancia() > distancias.get(atual)) {
                continue;
            }
            ...
            for (Vertice vizinho : getAdjacenciasOrdenadas(atual)) {
                int peso = obtemPesoEntre(atual, vizinho);
    ...
    public List<List<String>> componentesConexos() {
        List<List<String>> componentes = new ArrayList<>();
        Set<Vertice> visitados = new HashSet<>();

        List<Vertice> verticesOrdenados = vertices.stream()
    ...
    public int encontraComprimentoCaminho(String... caminho) {
        ...
        for (int i = 0; i < caminho.length - 1; i++) {
            ...
            Optional<Aresta> aresta = obtemArestasParaVizinho(origem, destino).stream().findFirst();
            if (aresta.isPresent()) {
                ...
                Integer peso = aresta.get().getPeso();
                comprimento += peso != null ? peso : 1;
            }
        }
        return comprimento;
    }
    ...
    private List<Vertice> getAdjacenciasOrdenadas(Vertice vertice) {
        return vertice.getAdjacencias().stream()
                .sorted(Comparator.comparing(Vertice::getNome))
                .toList();
    }

    private int obtemPesoEntre(Vertice atual, Vertice vizinho) {
        List<Aresta> arestasEncontradas = obtemArestasParaVizinho(atual, vizinho);

        if (arestasEncontradas.isEmpty()) {
            return 1;
        }

        Integer peso = arestasEncontradas.get(0).getPeso();
